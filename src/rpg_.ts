...
import { UserSecure, Contents, Utils } from "RTTRPG/modules";

type Bot = BotManager.Bot;
...
function makeSelection(msg: Message, user: User, unit: Unit, selections) {
...
function battle(msg: Message, user, unit) {
  msg.reply(
    Utils.Strings.format("전투 발생!\n{0} vs {1}", [user.id, unit.name])
  );
...
const battleSelection = [
  {
    desc: "공격하기",
    func: (msg: Message, user: User, target: Unit) => {
      if (user.items.weapon.cooltime > 0) {
        msg.reply(
          Utils.Strings.format(
            "무기 쿨타임: {0}s",
            user.weapon.cooltime.toFixed(2)
          )
        );
        makeSelection(msg, user, target, battleSelection);
        return;
      }
      let weapon = Contents.Items.find(user.items.weapon.id);
      if (!weapon) {
      ...
      if (user.items.weapon.health >= 0) {
        if (user.items.weapon.health > 1) user.items.weapon.health--;
          ...
          //TODO TypeScript화. 더이상 못참겠다
          //TODO 개수가 여러개인 무기가 체력을 공유함 -> 개수가 여러개면 hp를 다시 채우자!!
          msg.reply("무기 {0}(이)가 파괴되었습니다!".format(weapon.name));
          //if(user.items.items.find(entity=>entity.id==))
...
function onMessage(msg: BotManager.Message) {
  ...
  const hash: number = BotManager.JavaPackage.java.lang
    .String(msg.author.avatar.getBase64())
    .hashCode();
  const user: UserSecure.User = users.find((u) => u.hash == hash);
